				...
				if(esModoNuevo){
					...
					for(ModeloMarca tipo : lstTiposMunicion){
						if(tipo.getId().equals((Long)item.getItemProperty("tipo.id").getValue())){
							...
							lstMarcas = modeloMarcaService.buscarHijos(tipo);
						}
					}
					
					for(ModeloMarca tipo : lstMarcas){
						if(tipo.getId().equals((Long)item.getItemProperty("marca.id").getValue())){
							cmbMuniMarca.select(tipo);
						}
					}
