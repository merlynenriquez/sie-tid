...
public class PanelRegistroParteInmueble extends CustomComponent implements ClickListener  {
	...
	public void postConstruct() {
		...
		List<Inmueble> lInmuebles = null;// inmuebl.listarOrganizaciones();
		cmbInmInmueble.setContainerDataSource(new BeanItemContainer<Inmueble>(Inmueble.class,  lInmuebles));
		cmbInmInmueble.setItemCaptionPropertyId("nombre");
		...
		tblInmuebles.addListener(new ValueChangeListener() {
			...
			public void valueChange(ValueChangeEvent event) {
				...
				/*	instalacion.setId((Long) item.getItemProperty("id").getValue());
					txtAlto.setValue(item.getItemProperty("alto").getValue());
					txtAncho.setValue(item.getItemProperty("ancho").getValue());
					txtLargo.setValue(item.getItemProperty("largo").getValue());
					txtLatitud.setValue(item.getItemProperty("latitud").getValue());
					txtLongitud.setValue(item.getItemProperty("longitud").getValue());
					txtNombre.setValue(item.getItemProperty("nombre").getValue());
					txtRadio.setValue(item.getItemProperty("radio").getValue());
					txtZonaProduccion.setValue(item.getItemProperty("zonaProduccion").getValue());
					...
					BeanItemContainer<Valor> clstTipoLetrado = (BeanItemContainer<Valor>)cmbTipoInstalacion.getContainerDataSource();
					clstTipoLetrado.get
					for (Valor tipoLetrado : lstTipoLetrado) {
						if (tipoLetrado.getId().equals((Long) item.getItemProperty("tipo.id").getValue())) {
							cmbTipoInstalacion.select(tipoLetrado);
							break;
						}
					}
					...
					for (Valor tipoLetrado : lstTipoLetrado) {
						if (tipoLetrado.getId().equals((Long) item.getItemProperty("tipo.id").getValue())) {
							cmbSituacion.select(tipoLetrado);
							break;
						}
					}
					*/
	...
	public void buttonClick(ClickEvent event) {
		...
		if(event.getButton().equals(btnInmAgregar)){
			...
			logger.debug( "Grabar es nuevo ? " + inmueble.esNuevo() );
	...
	private void cargarTablaInmuebles( ){
		...
		List<DetPerInmExp> lstinmuebles = null;//instalacionService.buscar(instalacion);
		int con=1;
