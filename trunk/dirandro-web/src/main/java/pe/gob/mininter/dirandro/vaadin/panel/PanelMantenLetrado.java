...
public class PanelMantenLetrado extends DirandroComponent implements TextChangeListener,ClickListener {
	...
	public PanelMantenLetrado(List<Opcion> acciones, String height) {
		...
		this.valorService = Injector.obtenerServicio(ValorService.class);
		this.letradoService = Injector.obtenerServicio(LetradoService.class);		
		this.personaService = Injector.obtenerServicio(PersonaService.class);
	...
	public void postConstruct() {
		...
		pnlDatos1_2.setEnabled(false);
		pnlDatos1_3.setEnabled(false);
		...
		tblLetrados.addListener(new ValueChangeListener() {
			...
			public void valueChange(ValueChangeEvent event) {
				...
				if(esModoNuevo){
					...
					txtTipoDocumento.setValue(letrado.getPerPersona().getTipoDocumento().getNombre());
					txtNumeroDocumento.setValue(letrado.getPerPersona().getNroDocumento());
					txtSexo.setValue(letrado.getPerPersona().getSexo().equalsIgnoreCase("F") ? "Femenino" : "Masculino");
		...
		cmbNombres.addListener( new ValueChangeListener() {
			...
			private void cambiaPersona(ValueChangeEvent event) {
				...
				if( cmbNombres.getValue() != null) {
					...
					txtTipoDocumento.setValue(perSel.getTipoDocumento().getNombre());
					txtSexo.setValue(perSel.getSexo().equalsIgnoreCase("F") ? "Femenino" : "Masculino");
					txtNacionalidad.setValue(perSel.getNacionalidad().getNombre());
				}
			}			
		});
	}
	...
	public void obtenerDatosPersona() {
		lstPersonas = personaService.buscar(null);
		this.cmbNombres.setContainerDataSource(new BeanItemContainer<Persona>(Persona.class, lstPersonas));
		this.txtTipoDocumento.setValue(personaPopUp.getTipoDocumento().getNombre());
		this.txtNumeroDocumento.setValue(personaPopUp.getNroDocumento());
		this.txtSexo.setValue((personaPopUp.getSexo().equalsIgnoreCase("F")) ? "Femenino" : "Masculino");
		this.txtNacionalidad.setValue(personaPopUp.getNacionalidad().getNombre());
		
		for (Persona persona : lstPersonas) {
			if (persona.getId().equals(personaPopUp.getId()))
				this.cmbNombres.select(persona);
		}
	}
	...
	private void registrarLetrado(){
		...
		letrado.setPerPersona((Persona)cmbNombres.getValue());
	...
	private HorizontalLayout buildPnlDatos1_1() {
		...
		pnlDatos1_1.addComponent(cmbNombres);
