...
public class PanelMantenDependencia extends DirandroComponent implements ClickListener {
		...
		treeDependencia.addListener(new ValueChangeListener() {
			...
			public void valueChange(ValueChangeEvent event) {
				...
				if(esModoNuevo){
					...
					cmbPadre.select((Dependencia) item.getItemProperty("padre").getValue());
	...
	private void recorrer(String nombre, Map<String, List<Dependencia>> map, Long objPadre, HierarchicalContainer hwContainer) {//ok		
		...
		for (Dependencia dependencia : map.get(nombre)) {			
			...
			item.getItemProperty("estado.id").setValue(dependencia.getEstado() != null ? dependencia.getEstado().getId() : null);
	...
	public void buttonClick(ClickEvent event) {
		if(event.getButton().equals(btnCrear)){
			...
			if(flagNuevaDependencia){
				...
				sbContenido.append("Dependencia : ").append(dependencia.getNombre()).append(" se ha "+ Constante.OPERACION.CREAR_REGISTRO+" satisfactoriamente.");
			}
			else{
				Item item = treeDependencia.getItem(treeDependencia.getValue());
				dependencia.setId(Long.parseLong(item.getItemProperty("id").getValue().toString()));				
				dependenciasService.actualizar(dependencia);
				sbContenido.append("Dependencia : ").append(dependencia.getNombre()).append(" se ha "+ Constante.OPERACION.ACTUALIZAR_REGISTRO+" satisfactoriamente.");
			}
			
			cargarDependenciaCombo();
			
			AlertDialog alertDialog = new  AlertDialog("Dependencia", sbContenido.toString(), "Aceptar", "400");
